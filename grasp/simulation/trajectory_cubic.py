...
def traj_planning_ALL(x,Hz):
    ...
    for i in range(n-1):
        ...
        traj_pos_, traj_vel_, traj_acc_, t_ = traj_planning(x1, v1, x2, v2, last_dt, dt, t_num)
        if i == 0:
            traj_pos = traj_pos_
            traj_vel = traj_vel_
            traj_acc = traj_acc_
...
def traj_planning(x1, v1, x2, v2, last_dt, dt, t_num):
    ...
    """
    Theta = np.array( [[x1[0], x2[0], v1[0], v2[0]],
                       [x1[1], x2[1], v1[1], v2[1]], 
                       [x1[2], x2[2], v1[2], v2[2]]] )
    ...
    a00 = Theta[0,0]
    a01 = Theta[0,2]
    a02 = (-3/dt**2)*Theta[0,0] +  (3/dt**2)*Theta[0,1] +   (-2/dt)*Theta[0,2] +   (-1/dt)*Theta[0,3]
    a03 =  (2/dt**3)*Theta[0,0] + (-2/dt**3)*Theta[0,1] + (1/dt**2)*Theta[0,2] + (1/dt**2)*Theta[0,3]

    a10 = Theta[1,0]
    a11 = Theta[1,2]
    a12 = (-3/dt**2)*Theta[1,0] +  (3/dt**2)*Theta[1,1] +   (-2/dt)*Theta[1,2] +   (-1/dt)*Theta[1,3]
    a13 =  (2/dt**3)*Theta[1,0] + (-2/dt**3)*Theta[1,1] + (1/dt**2)*Theta[1,2] + (1/dt**2)*Theta[1,3]
    
    a20 = Theta[2,0]
    a21 = Theta[2,2]
    a22 = (-3/dt**2)*Theta[2,0] +  (3/dt**2)*Theta[2,1] +   (-2/dt)*Theta[2,2] +   (-1/dt)*Theta[2,3]
    a23 =  (2/dt**3)*Theta[2,0] + (-2/dt**3)*Theta[2,1] + (1/dt**2)*Theta[2,2] + (1/dt**2)*Theta[2,3]
